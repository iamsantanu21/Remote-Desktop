...
server_socket.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, 10**6)  # Increase buffer size

print("✅ Server is running... Waiting for connections.")

clients = set()  # Store connected clients
PACKET_SIZE = 1400  # Max UDP safe packet size
FRAME_DELAY = 1 / 20  # 20 FPS (50ms delay)
...
def send_screen():
    ...
    while True:
        try:
            ...
            time.sleep(FRAME_DELAY)  # Slow down frame rate for stability
...
def process_command(command):
    ...
    try:
        ...
        if action == "MOUSE_MOVE":
            ...
            time.sleep(0.02)  # Small delay to prevent jumping back
            mouse.position = (x, y)
